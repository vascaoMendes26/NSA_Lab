...
Para analisar teoricamente o CFC OTA, usámos como recurso uma montagem que já nos era conhecida, o convencional amplificador Folded Cascode, mostrado
na Figura \ref{fig:Cascode}, o CFC parte da mesma topologia, mas tem algumas modificações levando a grandes melhorias.

O CFC cascode tem a grande vantagem de ter uma faixa de entrada Rail-to-Rail, e ainda é possível obter o dobro do ganho. 
...
\begin{figure}[h!]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Images/Circ_Analisado_Teorica.png}
    \caption{CFC OTA com circuito simplificado realçado}
        \label{fig:Circ_Simp}
    \end{subfigure}
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Images/Simplificadao.png}
    \caption{Circuito Simplificado}
        \label{fig:SimplificadoMaximo}
    \end{subfigure}
    
    \caption{Simplificação realizada para facilitar a Análise Teórica}
...
Nas subsecções abaixo constaram as explicações de como foram retiradas as expressões e os valores que foram mencionados ao longo 
do relatório.
...
\begin{align}
    GBW = \frac{G_m}{2\pi \times C_{out}}\, [Hz]
    \label{eq:GBWFinal}
\end{align}
...
consiste em averiguar todas as capacidades ligadas a esse nó, essas capacidades podem ser vistas na Figura \ref{fig:Condensadores}

\begin{figure}[htp]
    \centering
    \includegraphics[width=4cm]{Images/Cout.png}
    \caption{Condensadores Parasitas ligados ao nó de saída}
    \label{fig:Condensadores}
\end{figure}


Dito isto, pela análise do circuito acima, obtivémos a seguinte expressão para $C_{out}$ está representada na Equação \ref{eq:Cout}:

\begin{align}
    C_{out} = C_{gd4P} + C_{db4P} + C_{gd6N} + C_{db6N} + C_{Load}\,[F]
    \label{eq:Cout}
\end{align}
...
dos nós internos do circuito, mais concretamente, os nós que correspondem às fontes, como é possível ver na Figura \ref{fig:SimplificadoMaximo} está desenhado onde estão localizados estes dois polos,
estes que se encontram ligados aos transistores M4 e M6, respetivamente. A sua determinação é importante para garantir a estabilidade do
circuito em malha fechada.
Para entender qual dos nós tem a frequência mais baixa, correspondente ao 2º polo,
analisámos o circuito e vimos que o transistor $M_4$ é um PMOS tendo uma maior transcondutância e $M_6$ um NMOS tendo menor transcondutância, após desenvolver
as equações \ref{eq:fp2}, \ref{eq:fp3} e \ref{eq:Rp2} \ref{eq:Rp3} abaixo no papel, foi fácil de notar que o segundo polo está associado ao nó do transistor $M_4$
e o terceiro polo associado ao nó do transistor $M_6$, na Figura \ref{fig:polos} é possível ver as capacidades parasitas ligadas a cada nó, onde após este desenho foi possível retirar as expressões abaixo.


\begin{figure}[htp]
    \centering
    \includegraphics[width=6.5cm]{Images/Polo2_Polo3.png}
    \caption{Esquemático para facilitar o cálculo das capacidades}
    \label{fig:polos}
\end{figure}


As expressões para as frequências do segundo e terceiro polo são dadas por:
...
Para continuar a análise, é necessário realizar alguns cálculos intermédios para determinar as expressões equivalentes para $R_{p2}$, $C_{p2}$, $R_{p3}$ e $C_{p3}$.
...
Nas equações \ref{eq:Rp2} e \ref{eq:Rp3}, o fator $(2-BE)$ justifica-se pela presença do efeito de corpo nos transístores $M_4$ e $M_6$.

Como estes transístores estão na configuração de common-gate, o efeito de corpo influencia, aumentando a transcondutância dos mesmos.
 Matematicamente, isto reflete-se no denominador da equação, resultando num valor de resistência menor do que seria num transístor ideal sem este efeito.
...
Este parâmetro do circuito tem em conta as quedas de tensão 
...
\subsection{Fator de excesso de ruído}
O fator de excesso de ruído (ENF) do circuito é um parâmetro que
indica o excesso de ruído criado por um amplificador de ganho $A_{v}$.
Um amplificador com ENF igual a 1 significa que o mesmo não acrescenta 
nenhum ruído ao sinal, ou seja, o ruído na saída não tem nenhuma contribuição
relativa ao ruído gerado pelo próprio amplificador.
Qualquer valor de ENF superior a 1 indica que o amplificador acrescenta
ruído ao sinal, que é o caso que se verifica na realidade.

O fator de ruído foi estudado para metade do circuito do CFC OTA, 
tendo em conta que foi realizada uma aproximação e não foram contabilizados os excessos de ruído dos transistores em configuração de Cascode. 
A expressão obtida para o ENF é a seguinte:
