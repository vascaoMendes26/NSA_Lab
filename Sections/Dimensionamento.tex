...
De acordo a análise, foi desenvolvido um script 
...
calcular a razão de tamanho de cada transístor.
...
tem que estar próximo de $62dB$. Para atingir este valor, foi necessário ver quais os transístores que influenciam este parâmetro,
a conclusão pode ser vista na Tabela \ref{tab:gainParameters} \newline\newline
...
\begin{figure}[htp]
    \centering
    \includegraphics[width=8cm]{Images/GainDCGraph.png}
...
\subsection{Análise do Fator de excesso de ruído}
...
\begin{figure}[htp]
    ...
    \caption{Fator de excesso de ruído em função de $V_{DSsat}$}
    \label{fig:NFGraph}
\end{figure}
...
Numa fase posterior, viu-se a necessidade de reduzir este valor para $I_{B} = \SI{80}{\micro\ampere}$, de forma
...
na qual se encontra na tabela~\ref{tab:finalSizing}. Os valores definidos abaixo foram obtidos através da escolha de uma corrente de $I_b = 80 \text{$\mu$A}$
...
\begin{table}[h!]
    ...
    \begin{tabular}{cccc}
        \toprule[1.2pt]
        \textbf{Transístor} & \textbf{Length(L)} & \textbf{Width(W)} & \textbf{VDSsat}\\
        \hline
        $M_{2aN}$ & $\SI{1}{\micro\meter}$ & $\SI{90.30}{\micro\meter}$ & $\SI{55}{\milli\volt}$\\[0.25cm]

        $M_{2bP}$ & $\SI{1}{\micro\meter}$ & $\SI{90.30}{\micro\meter}$ & $\SI{55}{\milli\volt}$\\[0.25cm]
...
\begin{table}[h!]
    ...
    \begin{tabular}{ccc}
        \toprule[1.2pt]
        \textbf{Transístor} & \textbf{Length(L)} & \textbf{Width(W)}\\
        \hline
        $M_{B1P}$ & $\SI{800}{\nano\meter}$ & $\SI{4.535}{\micro\meter}$\\[0.25cm]

        $M_{B2P}$ & $\SI{800}{\nano\meter}$ & $\SI{4.535}{\micro\meter}$\\[0.25cm]

        $M_{B3N}$ & $\SI{800}{\nano\meter}$ & $\SI{6.53}{\micro\meter}$\\[0.25cm]
...
Dados os valores da tabela~\ref{tab:finalSizing}, o resultado teórico obtido para circuito é dado na seguinte tabela~\ref{tab:expectedResults}.


\begin{table}[h!]
    \centering
    % --- ESTA LINHA É QUE FAZ APARECER NO ÍNDICE ---
    \caption{Resultados Obtidos no script em Python} 
